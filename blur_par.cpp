...
void blur_helper (unsigned char *res, unsigned char *frame, unsigned char *blur_vector,
    ...
    for (int row = start_row; row < end_row; row++) {
        for (int col = start_col; col < end_col; col++) {
            ...
            char r = frame[center_pixel_index + 0];
            char g = frame[center_pixel_index + 1];
            char b = frame[center_pixel_index + 2];
...
void blur_parallel (unsigned char *res, unsigned char *frame, unsigned char *blur_vector,
    ...
    args.res = res;
    args.frame = frame;
    args.blur_vector = blur_vector;
    args.frame_width = frame_width;
    args.frame_height = frame_height;
    args.blur_width = blur_width;
    args.blur_height = blur_height;
    args.start_row = 0;
    args.end_row = frame_height / 2;
    args.start_col = 0;
    args.end_col = frame_width;

    pthread_create(&thread_id, NULL, blur_thread_start, &args);
    blur_helper (res, frame, blur_vector, frame_width, frame_height,
            blur_width, blur_height, frame_height/2, frame_height, 0, frame_width);
